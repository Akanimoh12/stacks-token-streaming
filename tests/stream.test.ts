...
describe("test token streaming contract", () => {
  ...
  it("ensures recipient can withdraw tokens over time", () => {
    ...
    // Block 2 was used to create stream in beforeEach
    // Block 3 was used by previous test
    // `withdraw` will be called in Block 4
    // so expected to withdraw (Block 4 - Start_Block) = (4 - 0) tokens
  ...
  it("signature verification can be done on stream hashes", () => {
    ...
    // Convert Clarity buffer value to Uint8Array
    const hashValue = cvToValue(hashedStream0.result);
    const hashAsHex = Buffer.from(hashValue as Uint8Array).toString("hex");
